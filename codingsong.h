class CodingSong
{
	...
	void toFile()
	{
		...
		HENCODE enc;
		HSTREAM hstr;
		hstr = BASS_StreamCreate( 48000, 2, BASS_STREAM_DECODE, STREAMPROC_PUSH, 0 );
		enc = BASS_Encode_Start( hstr, "oggenc -q 8 - -o output.ogg", 0, 0, 0 );
		int n;
		for( i = 0; i < songlen; )
		{
			for( n = 0; n < 5000; n += 2 )
			{
				s[n] = (short)(left[i] * 32700.0);
				s[n+1] = (short)(right[i] * 32700.0);
				i++;
			}
			BASS_StreamPutData( hstr, s, 10000 );
			BASS_ChannelGetData( hstr, s, 10000 );
		}
		BASS_Encode_Stop( enc );
		BASS_StreamFree( hstr );
	}
};
